...
export default function EditProgram() {
    ...
    const program = getProgramByName(db, editProgram!);
    const originalName = program.name;
    
    let counter = 0;
    const [programData, setProgramData] = useState<programDataType>({
        name: program.name, 
        days: program.days.map((dayName) => ({
            id: counter++,
            name: dayName,
            color: dayNamesColors.find(d => d.name === dayName)?.color
        }))
    });
    ...
    const HeaderComponent = () => (
        <View className={'p-4'}>
            <TouchableOpacity className={'h-15 bg-red-500 mb-4 p-3 w-20 self-end'} onPress={() => {setEditProgram(null)}}>
        ...
        <View className={'p-4'}>
            ...
            <TouchableOpacity className={'h-15 bg-green-500 mb-4 p-3 w-full'}
                onPress={() => {
                    ...
                    const result = replaceProgram(db, originalName, programData.name.trim(), dayNames);
                    if (result == 'success') {
                        Toast.show({
                            type: 'success',
                            text1: 'Success',
